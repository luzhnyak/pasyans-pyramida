...
function createDeck(){
	var d=[["Clovers", "2",  2], ["Clovers", "3", 3], ["Clovers", "4", 4], ["Clovers", "5", 5], ["Clovers", "6", 6], ["Clovers", "7", 7], ["Clovers", "8", 8], ["Clovers", "9", 9], ["Clovers", "10", 10], ["Clovers", "J", 11], ["Clovers", "Q", 12], ["Clovers", "K", 13], ["Clovers", "A", 1], ["Hearts", "2", 2], ["Hearts", "3", 3], ["Hearts", "4", 4], ["Hearts", "5", 5], ["Hearts", "6", 6], ["Hearts", "7", 7], ["Hearts", "8", 8], ["Hearts", "9", 9], ["Hearts", "10", 10], ["Hearts", "J", 11], ["Hearts", "Q", 12], ["Hearts", "K", 13], ["Hearts", "A", 1], ["Pikes", "2", 2], ["Pikes", "3", 3], ["Pikes", "4", 4], ["Pikes", "5", 5], ["Pikes", "6", 6], ["Pikes", "7", 7], ["Pikes", "8", 8], ["Pikes", "9", 9], ["Pikes", "10", 10], ["Pikes", "J", 11], ["Pikes", "Q", 12], ["Pikes", "K", 13], ["Pikes", "A", 1], ["Tiles", "2", 2], ["Tiles", "3", 3], ["Tiles", "4", 4], ["Tiles", "5", 5], ["Tiles", "6", 6], ["Tiles", "7", 7], ["Tiles", "8", 8], ["Tiles", "9", 9], ["Tiles", "10", 10], ["Tiles", "J", 11], ["Tiles", "Q", 12], ["Tiles", "K", 13], ["Tiles", "A", 1]];		
	
	while (d.length>0) {
		var rnd=getRandomInt(0, d.length);
		
		deck.push({mast:d[rnd][0],rist:d[rnd][1],activ:false,ochko:d[rnd][2],remov:false});
		d.splice(rnd,1);			
...
function addCards(i,x,y){
	...
	cards[i].style.backgroundImage="url(images/"+deck[i].mast+"-"+deck[i].rist+".png)";	
	cards[i].style.left=x+"px";
	cards[i].style.top=y+"px";
	cards[i].onclick=function() {clickCards(i)};
	...
	if (deck[i].activ) {
		if (chekA==0) {
			...
			if (deck[i].ochko+deck[chekA].ochko==13) {
				...
				changeActiv();			
				chekA=0;	
				if (chekA==1 || i==1) {alert("Перемога!");}
			}
					
		}			
	}else if (i>28) {
				x=120;
				cards[lastB].style.backgroundImage="url(images/"+deck[lastB].mast+"-"+deck[lastB].rist+".png)";	
...
function loadGame(){
	...
	startGame.onclick=function() {game.removeChild(winLoad)};
...
function newGameLoad () {
	...
	for (var i = 1; i <= 52; i++) {
		deck[i].activ=false;
		if (!deck[i].remov) {game.removeChild(cards[i]);}
	}
